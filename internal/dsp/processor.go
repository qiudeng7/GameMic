...
func (p Params) Normalized() Params {
	...
	if math.IsNaN(p.ThresholdDB) || math.IsInf(p.ThresholdDB, 0) {
		p.ThresholdDB = -50
	}
...
func (p *Processor) Sample(input float32) float32 {
	...
	if level > p.envelope {
		envRate = 1.0 / (0.001 * SampleRate)
	}
	p.envelope += envRate * (level - p.envelope)
	if p.envelope >= p.threshold {
		p.hold = int(0.120 * SampleRate)
	} else if p.hold > 0 {
		p.hold--
	}
	gateTarget := 0.0
	if !p.gateEnabled || p.hold > 0 {
		gateTarget = 1
	}
	gateRate := 1.0 / (0.060 * SampleRate)
	if gateTarget > p.gate {
		gateRate = 1.0 / (0.002 * SampleRate)
	}
	...
	required := 1.0
	if math.Abs(y) > 0.95 {
		required = 0.95 / math.Abs(y)
	}
	if required < p.limiter {
		p.limiter = required
	} else {
		p.limiter += (required - p.limiter) / (0.080 * SampleRate)
	}
